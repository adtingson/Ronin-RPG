...
function applyAllyBuff(target) {
    if (target.occupation == "Mentor") {
        if (ronin.technique.some(technique => technique.id == target.technique.id)) {
            ...
            interactText.innerHTML += `${ronin.name} spent some time under the guidance of ${target.name}. ${ronin.name} learns ${target.technique.desc}!<br>`;
        }
    }
...
function weaponsDeliveryCheck() {
    if (weaponsDelivery.length) {
        ...
        interactText.innerHTML += `As ${ronin.name} arrives at the city, ${ronin.gender == "Male" ? "he" : "she"} gets approached by a courier. The courier delivers the special weapons from ${ronin.name}'s blacksmith allies. Each type of weapon is crafted better than the last: ${weaponsDelivery.join(", ")}<br>`;
